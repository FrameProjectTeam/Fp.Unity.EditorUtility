...
using Fp.ProjectTwiner.Utility.ReorderableList.Runtime.Attributes;
...
namespace Fp.ProjectTwiner.Utility.ReorderableList.Editor
{
	...
	public class ReorderableDrawer : PropertyDrawer
	{
		...
		private static readonly Dictionary<int, ReorderableList> lists = new Dictionary<int, ReorderableList>();
			...
			if(array != null && array.isArray)
			{
				if(!lists.TryGetValue(id, out list))
				{
					if(attrib != null)
					{
						...
						ReorderableList.ElementDisplayType displayType =
							attrib.singleLine ? ReorderableList.ElementDisplayType.SingleLine : ReorderableList.ElementDisplayType.Auto;

						list = new ReorderableList(
							array, attrib.add, attrib.remove, attrib.draggable, displayType, attrib.elementNameProperty, attrib.elementNameOverride, icon
						);
